...
class _HistDataHandler(object):
    ...
    def msgHandler(self,msg):
        ...
        if len(msg.date) > 8:
            self._timestamp.append((dt.datetime.strptime(msg.date,timeFormat).replace(tzinfo=tzlocal.get_localzone())).astimezone(timezone('EST')))
        else:
...
class Downloader(object):
    ...
    def requestData(self,contract,endDateTime,durationStr='1 D',barSizeSetting='30 secs',whatToShow='TRADES',useRTH=1,formatDate=1):  
        ...
        while self._timeKeeper.nrRequests(timeSpan=600) > 59:
            print 'Too many requests done. Waiting for 10 mins... '
            time.sleep(50)
            self.tws.disconnect() 
            time.sleep(550)
            self.tws.connect()
    ...
    def getIntradayData(self,contract, dateTuple):
        ...
        t = dt.datetime(*dateTuple).replace(hour=23, minute=0, second=0, microsecond=0)
        #openTime = ESTday.replace(hour=8, minute=0, second=0, microsecond=0)
        #closeTime = ESTday.replace(hour=17, minute=0, second=0, microsecond=0)
        #timeRange = pandas.date_range(openTime,closeTime,freq='30min')
        
        datasets = []

        t=t.strftime(timeFormat)+' EST'
        datasets.append(self.requestData(contract,t))
        
        #for t in timeRange:
        #    print t.strftime(timeFormat)
        #    datasets.append(self.requestData(contract,t.strftime(timeFormat),durationStr='1800 S'))
        #hist date seems not right
